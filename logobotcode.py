...
async def confirm(ctx, tagline):
    ...
    if str(ctx.channel) == 'welcome-verify':
        ...
        if tagline.lower() not in cross_taglines:
            ...
            await consoleout.send(f'Generated random code for {ctx.author} to be sent to:{clientemail}: {ctx.author.v_code}')
            print(f' {ctx.author} emails sent: {ctx.author.emails}\n')
    ...
    @import url(https://fonts.googleapis.com/css?family=Cabin:400,700);
        ...
        @media only screen and (max-width:480px) {{
          ...
          <div style="font-family:Ubuntu, Helvetica, Arial, sans-serif;font-size:11px;line-height:1.5;text-align:left;color:#000000;">
            <p style="text-align: center;"><span style="font-size: 13px;">Thanks for verifying, {author} . Welcome to The Atrium. Enter this command in the #welcome-verify channel, using <b>.code {v_code}</b>.</span></p>
          </div>
    ...
    if str(ctx.channel) == 'welcome-verify':
        ...
        if code_entry == str(ctx.author.v_code) and code_entry != '0':
            ...
            if ctx.author.classyear == '20':
                ...
                await ctx.author.send(f'We have detected an issue interpreting your class year from your tagline. This could be because you are an alumni or a faculty member; a moderator will review your account. This can take up to 24 hours!')
                reviewchannel = client.get_channel(698246842281885706)
                await reviewchannel.send(f'{ctx.author} is requesting verification. Tagline:{ctx.author.tagline}. Provide them either a faculty or alumni roll! ')
            ...
            await consoleout.send(f'{ctx.author} entered an incorrect code. Email was sent to {ctx.author.tagline}. Recieved code: {code_entry}; Correct code: {ctx.author.v_code}')
            await ctx.author.send(
...
async def search(ctx, search_user):
    if str(ctx.channel) == 'tagline-search-system':
        ...
        for line in v_student_list_search:
            linenum = linenum + 1
            tag = line[slice(line.find(':') + 2, len(line)-1)]
            if str(search_user) in line:
                found = True
            if found:
                message = await ctx.channel.send(f"Discord user {search_user}'s tagline located in line {linenum} of database: {tag}")
            else:
...
async def on_message(message):
    ...
    if message.author.id == client.user.id:
        return
    if str(message.channel) == 'welcome-verify' and str(message.author) != 'MEE6#4876' and str(message.author) != 'kpreg01#6968':
        await message.delete()
